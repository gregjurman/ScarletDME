...
Private void ak_delete(DH_FILE *dh_file, short int akno, char id[],
                       short int id_len);

Private STRING_CHUNK *ak_read(DH_FILE *dh_file, short int akno, char id[],
                              short int id_len, bool read_data);

Private bool ak_write(DH_FILE *dh_file, short int akno, char id[],
                      short int id_len, STRING_CHUNK *rec);

Private DH_RECORD *rightmost(DH_TERM_NODE *node);

Private void copy_ak_record(DH_FILE *dh_file, DH_RECORD *rec_ptr,
                            short int base_size, char *id, short int id_len,
                            long int big_rec_head, long int data_len,
                            STRING_CHUNK *str, short int pad_bytes);
...
Private short int compare(char *s1, short int s1_len, char *s2,
                          short int s2_len, bool right_justified, bool nocase);

Private bool update_internal_node(DH_FILE *dh_file, short int subfile,
                                  NODE *node_ptr, char *id1, short int id1_len,
                                  long int offset1, char *id2,
                                  short int id2_len, long int offset2,
                                  char *id3, short int id3_len,
                                  long int offset3);
...
void op_selright() { akscan(TRUE); }

void op_selleft() { akscan(FALSE); }
...
void op_setright() { setakpos(TRUE); }

void op_setleft() { setakpos(FALSE); }
...
Private short int create_ak(
  ...
  if (big_dict_rec) {
    ...
    do {
      ...
      n = (short int)
          min(dict_rec_len, DH_AK_NODE_SIZE - DH_ITYPE_NODE_DATA_OFFSET);
...
Private bool ak_write(DH_FILE *dh_file, /* File descriptor */
  ...
  if ((used_bytes == 0) || (used_bytes > DH_AK_NODE_SIZE)) {
    log_printf("AK_WRITE: Invalid byte count (x%04X) at subfile %d node "
               "%ld\nof file %s\n",
  ...
  while (rec_offset < used_bytes) {
    ...
    if (x <= 0) /* Record goes here */
      ...
      if (x == 0) /* Found key  -  Replace record */
        ...
        gap_required = (short int)(base_size - rec_size);
            /* Adjust space to overwrite */
        next_offset = rec_offset + rec_size;
      } else /* Gone past key value */
          {
        gap_required = (short int) base_size;
            /* Open a gap to hold this record */
  ...
  if (used_bytes + gap_required <= DH_AK_NODE_SIZE) {
    ...
    if (rec_offset < used_bytes) /* Something to move */
      ...
      if (tail->prev != NULL) /* Not updating the root terminal node */
          {
        if (!update_internal_node(dh_file, subfile, tail->prev,
                                  /* Node structure */
...
Private void copy_ak_record(DH_FILE *dh_file, DH_RECORD *rec_ptr,
                            ...
                            /* Head of big record chain or zero if not big */
                            long int data_len, /* Data bytes */
                            STRING_CHUNK *str,
                            /* Record data string chunk head */
                            short int pad_bytes) {
  char *data_ptr; /* Data */
...
Private long int get_ak_node(DH_FILE *dh_file, short int subfile) {
  ...
  if (ak_header->free_chain == 0) {
    ...
    new_node_num = (long int)((file_bytes - dh_file->ak_header_bytes) /
                                  DH_AK_NODE_SIZE + 1);
...
Private void ak_delete(DH_FILE *dh_file, /* File descriptor */
  ...
  do {
    ...
    if (tail->node.int_node.node_type != AK_INT_NODE) {
      log_printf("AK_DELETE: Invalid node type (x%04X) at subfile %d node "
                 "%ld\nof file %s\n",
                 (int)(tail->node.int_node.node_type), (int) subfile, node_num,
                 fptr->pathname);
      dh_err = DHE_AK_NODE_ERROR;
  ...
  used_bytes = tail->node.term_node.used_bytes;
  if ((used_bytes == 0) || (used_bytes > DH_AK_NODE_SIZE)) {
    log_printf("AK_DELETE: Invalid byte count (x%04X) at subfile %d node "
               "%ld\nof file %s\n",
               used_bytes, (int) subfile, node_num, fptr->pathname);
    dh_err = DHE_POINTER_ERROR;
    goto exit_ak_delete;
...
Private STRING_CHUNK *ak_read(
  ...
  do {
    ...
    if (node->int_node.node_type != AK_INT_NODE) {
      log_printf("AK_READ: Invalid node type (x%04X) at subfile %d node "
                 "%ld\nof file %s\n",

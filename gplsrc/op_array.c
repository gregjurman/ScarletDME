    ...
    if (!strcmp(block_name, "*VARS")) {
      ...
      if (add_block_to_common_chain) {
        if (!strcmp(block_name, "$SYSCOM")) {
          process.syscom = a_hdr;
              /* Remember $SYSCOM location for LDSYS opcode */
        }
...
void op_dimlcl() { a_dimension(FALSE, FALSE); }

void op_dimlclp() { a_dimension(TRUE, FALSE); }
...
ARRAY_HEADER *a_alloc(
  ...
  num_chunks =
      (short int)((elements + MAX_ARRAY_CHUNK_SIZE - 1) / MAX_ARRAY_CHUNK_SIZE);
    ...
    case ARRAY: /* Redimensioning existing array */
      ...
      if ((cols != a_hdr->cols) || (rows != a_hdr->rows)) {
        ...
        if ((a_hdr->flags & AH_PICK_STYLE) &&
            ...
            {
          k_error(sysmsg(1139));
              /* Pick style matrices must have at least one element */
        }
        ...
        if (diff < 0) /* Redimensioning smaller */
          ...
          n = (short int)((elements + MAX_ARRAY_CHUNK_SIZE - 1) /
                          MAX_ARRAY_CHUNK_SIZE);

          if (n > a_hdr->num_chunks) {
            mem_reqd =
                sizeof(struct ARRAY_HEADER) + ((n - 1) * sizeof(short int *));

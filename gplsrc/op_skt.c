 ...
 *               Re-wrote op_acceptskt(), op_srvraddr().  Should now
 * transparently handle IPv6.
 *
 * 03 Apr 09 gwb op_srvsckt() has pretty much been completely re-written.  It
 * now
 ...
 *               when creating new server sockets.  This was done in order to
 * make the
 *               GPL version work like the commercial version.
 *               This applies to the op_srvsckt() routine for now.  op_openskt()
 * is being
 *               examined.
 *
...
void op_srvrskt() {
  ...
  */
  ...
  if (flags & SKT_UDP) { /* SKT$UDP */
    hint.ai_socktype = SOCK_DGRAM;
        /* for the moment we're going to force datagram for UDP sockets... */
  }

  if (flags & SKT_ICMP) {
    hint.ai_socktype = SOCK_RAW; /* for the moment we're going to force raw
sockets for ICMP sockets... */
  }
...
exit_srvrskt:
  ...
  k_dismiss();
  k_pop(2); /* this was set to 1 for the old routine.  I'm still not quite sure
               how these work,
...
void close_skt(SOCKVAR *sock) { closesocket((SOCKET)(sock->socket_handle)); }

/* ======================================================================
   socket_wait()                                                          */

bool socket_wait(SOCKET skt, bool read, /* Read mode? */
                 int timeout) {
  fd_set socket_set;
  fd_set wait_set;
  struct timeval tm;

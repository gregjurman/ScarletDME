...
/* Internal functions */
Private char *SelectLeftRight(short int fno, char *index_name, short int listno,
                              short int mode);
...
#define MAX_ARGS 20
  ...
  for (i = 0; i < argc; i++) {
    ...
    bytes += 4 + ((arg_len + 1) & ~1);
        /* Four byte length + arg text (padded) */
  }
  va_end(ap);
...
char *DLLEntry QMChange(src, old, new, occurrences, start) char *src;
char *old;
char *new;
int occurrences;
int start;
...
void DLLEntry QMClearSelect(listno) int listno;
{
  struct {
    short int listno;
  } ALIGN2 packet;
...
void DLLEntry QMClose(fno) int fno;
{
  struct {
    short int fno;
  } ALIGN2 packet;
...
int DLLEntry QMConnect(host, port, username, password, account) char *host;
int port;
char *username;
char *password;
char *account;
...
int DLLEntry QMConnectLocal(account) char *account;
{
  int status = FALSE;
...
int DLLEntry QMDcount(src, delim_str) char *src;
char *delim_str;
{
  long int src_len;
  char *p;
...
void DLLEntry QMDebug(mode) bool mode;
{
...
char *DLLEntry QMDel(src, fno, vno, svno) char *src;
int fno;
int vno;
int svno;
{
...
void DLLEntry QMDelete(fno, id) int fno;
char *id;
...
void DLLEntry QMDeleteu(fno, id) int fno;
char *id;
...
bool DLLEntry QMEnterPackage(name) char *name;
{
  bool status = FALSE;
  int name_len;
...
bool DLLEntry QMExitPackage(name) char *name;
{
  bool status = FALSE;
  int name_len;
...
char *DLLEntry QMError() { return session[session_idx].qmerror; }

/* ======================================================================
   QMExecute()  -  Execute a command                                      */

char *DLLEntry QMExecute(cmnd, err) char *cmnd;
int *err;
{
...
char *DLLEntry QMExtract(src, fno, vno, svno) char *src;
int fno;
int vno;
int svno;
{
...
char *DLLEntry QMField(src, delim, first, occurrences) char *src;
char *delim;
int first;
int occurrences;
{
...
void DLLEntry QMFree(void *p) { free(p); }

/* ======================================================================
   QMGetSession()  -  Return session index                                */

int DLLEntry QMGetSession() { return session_idx; }

/* ======================================================================
   QMIns()  -  Insert field, value or subvalue                            */

char *DLLEntry QMIns(src, fno, vno, svno, new) char *src;
int fno;
int vno;
int svno;
char *new;
...
int DLLEntry QMLocate(item, src, fno, vno, svno, pos, order) char *item;
char *src;
int fno;
int vno;
int svno;
...
int DLLEntry QMLogto(account_name) char *account_name;
{
  bool status = FALSE;
  int name_len;
...
void DLLEntry QMMarkMapping(fno, state) short int fno;
short int state;
{
  struct {
    short int fno;
...
int DLLEntry QMMatch(str, pattern) char *str;
char *pattern;
{
...
char *DLLEntry QMMatchfield(str, pattern, component) char *str;
char *pattern;
int component;
{
  char template[MAX_MATCH_TEMPLATE_LEN + 1];
...
int DLLEntry QMOpen(filename) char *filename;
{
  int fno = 0;
...
char *DLLEntry QMRead(fno, id, err) int fno;
char *id;
int *err;
...
char *DLLEntry QMReadl(fno, id, wait, err) int fno;
char *id;
int wait;
int *err;
...
char *DLLEntry QMReadList(listno) int listno;
{
  char *list;
  short int status = 0;
  long int data_len = 0;
...
char *DLLEntry QMReadNext(listno) short int listno;
{
  char *id;
  long int id_len = 0;
  struct {
...
char *DLLEntry QMReadu(fno, id, wait, err) int fno;
char *id;
int wait;
int *err;
...
void DLLEntry QMRecordlock(fno, id, update_lock, wait) int fno;
char *id;
int update_lock;
int wait;
{
...
void DLLEntry QMRelease(fno, id) int fno;
char *id;
{
  int id_len;
  struct {
...
char *DLLEntry QMReplace(src, fno, vno, svno, new) char *src;
int fno;
int vno;
int svno;
char *new;
...
char *DLLEntry QMRespond(response, err) char *response;
int *err;
{
  long int reply_len = 0;
  char *reply;
...
void DLLEntry QMSelect(fno, listno) int fno;
int listno;
{
  struct {
    short int fno;
...
void DLLEntry QMSelectIndex(fno, index_name, index_value, listno) short int fno;
char *index_name;
char *index_value;
short int listno;
{
...
char *DLLEntry QMSelectLeft(fno, index_name, listno) short int fno;
char *index_name;
short int listno;
{ return SelectLeftRight(fno, index_name, listno, SrvrSelectLeft); }

char *DLLEntry QMSelectRight(fno, index_name, listno) short int fno;
char *index_name;
short int listno;
{ return SelectLeftRight(fno, index_name, listno, SrvrSelectRight); }

Private char *SelectLeftRight(fno, index_name, listno, mode) short int fno;
char *index_name;
short int listno;
short int mode;
{
...
void DLLEntry QMSetLeft(fno, index_name) short int fno;
char *index_name;
{ SetLeftRight(fno, index_name, SrvrSetLeft); }

void DLLEntry QMSetRight(fno, index_name) short int fno;
char *index_name;
{ SetLeftRight(fno, index_name, SrvrSetRight); }

Private void SetLeftRight(fno, index_name, mode) short int fno;
char *index_name;
short int mode;
{
  struct {
...
int DLLEntry QMStatus() { return session[session_idx].qm_status; }

/* ======================================================================
   QMWrite()  -  Write record                                             */

void DLLEntry QMWrite(fno, id, data) int fno;
char *id;
char *data;
...
void DLLEntry QMWriteu(fno, id, data) int fno;
char *id;
char *data;
...
Private bool context_error(expected) short int expected;
{
  char *p;
...
Private void delete_record(mode, fno, id) short int mode;
int fno;
char *id;
{
  int id_len;
...
Private char *read_record(fno, id, err, mode) int fno;
char *id;
int *err;
int mode;
{
...
Private void write_record(mode, fno, id, data) short int mode;
short int fno;
char *id;
char *data;
{
...
Private void Abort(msg, use_response) char *msg;
bool use_response;
{
...
Private char *memstr(str, substr, str_len, substr_len) char *str;
char *substr;
int str_len;
int substr_len;
{
...
Private bool match_template(string, template, component,
                            return_component) char *string;
char *template;
...
Private bool message_pair(type, data, bytes) int type;
char *data;
long int bytes;
{

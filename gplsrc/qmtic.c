...
int main(int argc, char *argv[]) {
  ...
  printf("#[ QMTIC %s   Copyright, Ladybridge Systems, %s.  All rights "
         "reserved. ]\n\n",
  ...
  if (terminfodir[0] == '\0') {
    ...
    if (terminfodir[0] == '\0')
      sprintf(terminfodir, "%s%cterminfo", sysdir, DS);
  }
  ...
  if (do_index) /* Index terminals */
    ...
    for (arg = 1; arg < argc; arg++) {
      if (argv[arg][0] != '-') {
        sprintf(src, "%s%c%c%c%s", terminfodir, DS, argv[arg][0], DS,
                argv[arg]);
        decompile_entry();
      }
    }
...
void process_escapes(p) u_char *p;
{
  short int i;
  short int n;
  u_char *q;
...
bool in_list(defn_names) char *defn_names; /* Names in definition */
{
  char name_list[512];
  char *p;
  char *q;
...
short int lookup(id, names, ct) char *id;
char *names[];
short int ct;
{
  short int i;
...
bool write_entry(name) char *name;
{
  int tgt_fu;
...
void build_index(decomp) bool decomp;
{
  DIR *dfu1;
  struct dirent *dp1;

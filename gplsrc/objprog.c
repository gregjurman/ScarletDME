...
Private bool find_name_map_entry(short int mode, OBJDATA *objdata, char *name,
                                 bool dismiss_obj);
Private bool find_undefined_name_handler(short int mode, OBJDATA *objdata,
                                         char *name, bool dismiss_obj);
...
void op_disinh() {
  ...
  */
  ...
  if (descr->type != OBJ)
    k_error(sysmsg(3460)); /* Object variable required */
    ...
    default:
      ...
      switch (p->type) {
        case ARRAY:
          ...
          if (arg_ct != i)
            k_error(sysmsg(1026));
                /* Argument count mismatch in object reference */
...
Private DESCRIPTOR *resolve_index(DESCRIPTOR *array_descr,
                                  /* The matrix being accessed */
                                  DESCRIPTOR *index_descr,
                                  /* The top index on the e-stack */

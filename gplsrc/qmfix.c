...
#define OffsetToNode(x)                                                        \
  ((long int)(((x) - ak_header_size) / DH_AK_NODE_SIZE + 1))
...
bool map_big_rec(short int rec_hdr_sf, int64 rec_hdr_offset, DH_RECORD *rec_ptr,
                 long int *map);
void tag_block(int64 offset, short int ref_subfile, int64 ref_offset,
               short int usage);
void map_node(short int sf, long int node, char *akmap, long int num_nodes,
              long int parent);
    ...
    do {
      ...
      *cmnd = '\0'; /* For quit key path which aborts gets() */
      ...
      if (cmnd[0] == '\0') {
        ...
        emit(
            "Wn=x       write hex value (1, 2 or 4 bytes in file byte order)\n");
...
void G_command(grp) long int grp;
{
  if (grp > 0) {
    if (subfile < 2)
      show_group(grp);
...
void L_command(id, id_len) char *id;
short int id_len;
{
...
void W_command(cmnd) char *cmnd;
{
  ...
  if (n == 2) /* Hex assignment */
    ...
    switch (n) {
      ...
      case 8:
        old_data =
            (old_data >> 24) | ((old_data & 0x00FF0000L) >> 8) |
            ((old_data & 0x0000FF00L) << 8) | ((old_data & 0x000000FFL) << 24);
        emit("Was %08lX\n", old_data);
        break;
    }
...
void show_window(addr) int64 addr;
{
    ...
    if (fu[sf] >= 0) {
      ...
      if (header.ak_map & (1 << akno)) /* Expected this AK subfile */
        ...
        if (fix()) {
          ...
          if (!write_header())
            goto exit_check;
          emit("   Corrected by removing reference to missing AK index "
               "subfile\n");
        }
      } else /* Did not expect this AK subfile */
          {
      ...
      if (released_group != 0) {
        ...
        if (fix()) {
          ...
          while (released_group < grp) {
            write_block(PRIMARY_SUBFILE, (((int64)(released_group - 1)) *
                                          group_bytes) + header_bytes,
      ...
      do {
        ...
        while (rec_offset < used_bytes) {
          ...
          if ((hgroup != grp) && (hash_errors < HASH_ERROR_LIMIT)) {
            hash_errors++;
            emit("Record '%s' mis-hashed into group %ld. Should be in group "
                 "%ld\n",
                 id, grp, hgroup);
...
long int get_overflow() {
  ...
  if ((offset = GetLink(header.params.free_chain)) != 0) {
    if (!read_block(OVERFLOW_SUBFILE, offset, BLOCK_HEADER_SIZE,
                    (char *)buff)) {
      offset = 0;
...
void show_group(group) long int group;
{
  short int sf = 0;
  int64 offset;
  short int used_bytes;
  ...
  000000000400: 0030 00 ........ 00000016
  nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
  000000000430: 0035 01 00002000 ........
  nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
  000000002000> .... .. 00002400 00000884
  000000002400> .... .. 00000000 ........
  000000000465: 1234 00 ........ 00000018
  nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
  */
...
Private void show_node(group) long int group;
{
...
bool open_subfile(sf) short int sf;
{
  char path[160 + 1];
...
bool delete_subfile(sf) short int sf;
{
  char path[160 + 1];
...
bool map_big_rec(rec_hdr_sf, rec_hdr_offset, rec_ptr, map) short int rec_hdr_sf;
...
void tag_block(offset, ref_subfile, ref_offset, usage) int64 offset;
...
void map_node(sf, node, ak_map, num_nodes, parent) short int sf;
long int node;
char *ak_map;
long int num_nodes;
long int parent;
        ...
        if (rec_ptr->flags & DH_BIG_REC) {
          ...
          while (big_offset != 0) {
            ...
            if (ak_map[big_node_no] != (char) - 1) {
              emit("Type %d node %ld also referenced as big record from node "
                   "%ld\n",
...
void map_ak_free_chain(sf, map, num_nodes) short int sf;
char *map;
long int num_nodes;
{
        ...
        if (rec_ptr->flags & DH_BIG_REC) {
          ...
          do {
            ...
            if (next_big_offset != 0) {
              big_buff->next = SetLink(overflow_offset + group_bytes);
                  /* 0526 */
            }
  ...
  do {
    ...
    while (rec_offset < used_bytes) {
      ...
      if ((id_len == rec_ptr->id_len) &&
        ...
        goto add_record;
#else
        emit("   Record '%s' not moved as a correctly hashed version also "
             "exists\n",
             id);
...
void event_handler(signum) int signum;
{

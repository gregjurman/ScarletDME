  ...
  if (src == NULL) {
    ...
    if (tgt->ref_ct == 1) {
      ...
      if (tgt->bytes < MAX_STRING_CHUNK_SIZE) {
        ...
        chunk_size = (short int) min(src_len, MAX_STRING_CHUNK_SIZE);
...
void op_substra() { substra(FALSE); }

void op_psubstra() { substra(TRUE); }
    ...
    if (tgt_hdr == NULL) /* Target is a null string */
      ...
      if (tgt_tail->bytes < MAX_STRING_CHUNK_SIZE) {
        ...
        if (len > tgt_tail->bytes + add_bytes)
          len = (short int)(tgt_tail->bytes + add_bytes);
...
Private STRING_CHUNK *copy_substr(
  ...
  bytes = (short int)(src_str->bytes - offset);
      /* Bytes remaining in source chunk */
  if (bytes > substr_len)
    bytes = (short int) substr_len;
  ...
  while (substr_len > 0) {
    if (bytes <= chunk_space) /* All fits current target chunk */
      ...
      if (chunk_space > substr_len)
        chunk_space = (short int) substr_len;

...
void op_create_dh() {
  ...
  */
  ...
  descr = e_stack - 2;
  GetInt(descr);
  creation_flags = (unsigned short)(
      (descr->data.value < 0) ? 0 : (descr->data.value & DHF_CREATE));
...
bool get_voc_file_reference(
    char *voc_name, bool get_dict_name,
...
Private void open_file(bool map_name) /* Map file name via VOC entry */
  ...
  */
  ...
  if (map_name) {
    ...
    name_len = k_get_c_string(filename_descr, mapped_name, MAX_PATHNAME_LEN);
        /* 0458 */
    k_dismiss();
  }
...
void op_readonly() { process.op_flags |= P_READONLY; }

  ...
  if (k_get_c_string(descr, call_name, MAX_PROGRAM_NAME_LEN) > 0) {
    ...
    if (strchr("*$_!", call_name[0]) == NULL) /* No prefix */
      ...
      if (mode == 0) {
        ...
        sprintf(pathname, "%s%c%s", private_catalogue, DS, mapped_name);
    ...
    if (mode == 0) {
      ...
      sprintf(pathname, "%s%cgcat%c%s", sysseg->sysdir, DS, DS, mapped_name);
      if (access(pathname, 0) == 0)
        mode = 3;
    }
  }
...
void op_gosub() {
  ...
  pc =
      c_base + (*pc | (((long) * (pc + 1)) << 8) | (((long) * (pc + 2)) << 16));
}
...
void op_ongosub() { computed_jump(TRUE, FALSE); }

/* ======================================================================
   op_ongosubp()  -  Computed gosub (Pick style)                          */

void op_ongosubp() { computed_jump(TRUE, TRUE); }

/* ======================================================================
   op_ongoto()  -  Computed goto                                          */

void op_ongoto() { computed_jump(FALSE, FALSE); }

/* ======================================================================
   op_ongotop()  -  Computed goto (Pick style)                            */

void op_ongotop() { computed_jump(FALSE, TRUE); }

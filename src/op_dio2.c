...
void op_fileinfo() {
  ...
  Key values:           Action                         Returns
      ...
      3 FL_TYPE         Check file type                DH:         FL_TYPE_DH
  (3)
                                                       Directory:  FL_TYPE_DIR
  (4)
                                                       Sequential: FL_TYPE_SEQ
  (5)
   ...
   1003 FL_PHYSBYTES    Physical file size             Size in bytes, excl
  indices
   ...
   1010 FL_JNL_FNO      Return journalling file no     File no, zero if not
  journalling
...
void op_ospath() {
  ...
  */
  ...
  switch (key) {
    ...
    case OS_UNIQUE
        : /* Make unique file name. Path variable holds directory name */

...
void op_writeseq() { writeseq(FALSE); }

void op_writeseqf() { writeseq(TRUE); }
...
Private void emit(FILE_VAR *fvar, char *p, short int bytes) {
  ...
  while (bytes) {
    ...
    if (block != sq_file->base) {
      ...
      if ((sq_file->bytes = (short int)
           Read(sq_file->fu, sq_file->buff, SEQ_BUFFER_SIZE)) < 0) {
        sq_file->bytes = 0;
      }
      sq_file->base = block;
    }
...
void close_seq(FILE_VAR *fvar) {
  ...
  if (sq_file != NULL) {
    ...
    if (sq_file->record_name != NULL) {
      (void)
          unlock_record(fvar, sq_file->record_name, sq_file->record_name_len);

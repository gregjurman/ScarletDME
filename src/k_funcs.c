...
double rounding[14] = { 0.5, 0.05, 0.005, 0.0005, 0.00005, 0.000005, 0.0000005,
                        0.00000005, 0.000000005, 0.0000000005, 0.00000000005,
                        0.000000000005, 0.0000000000005, 0.00000000000005 };
    ...
    do {
      ...
      while (src_bytes_remaining > 0) {
        if (tgt_bytes_remaining == 0) {
          ...
          tgt_str = next_tgt;
          tgt_str->string_len = total_src_bytes_remaining;
              /* Only right in... */
...
int k_get_c_string(DESCRIPTOR *descr, char *s,
...
void k_get_int(DESCRIPTOR *p) {
  ...
  switch (p->type) /* ++ALLTYPES++ */
    ...
    case FLOATNUM:
      if (fabs(p->data.float_value) > LONG_MAX)
        k_error(sysmsg(1220));
...
void k_num_to_str(DESCRIPTOR *p) {
  ...
  switch (p->type) /* ++ALLTYPES++ */
    ...
    case FLOATNUM:
      s_len = ftoa(p->data.float_value, process.program.precision, FALSE, s);
          /* 0352 */
      if (strchr(s, '.') != NULL) {
...
void k_release(DESCRIPTOR *p) {
  ...
  switch (p->type) /* ++ALLTYPES++ */
    ...
    case SUBR:
      ...
      if (--(((OBJECT_HEADER *)(p->data.subr.object))->ext_hdr.prog.refs) ==
          0) {
      }

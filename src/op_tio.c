...
Private bool tio_print_string(PRINT_UNIT *pu, char *q, short int bytes,
...
void op_dsp() { display_chunked_string(TRUE); }
...
void op_heading() { heading(TRUE); }

void op_headingn() { heading(FALSE); }
...
void op_keyin() { getkey(FALSE, -1); }

void op_keyinc() { getkey(TRUE, -1); }
...
void op_setpu() {
  ...
  */
  ...
  switch (key) /* !!PU!! */
    ...
    case PU_CPI:
      k_get_float(descr);
      pu->cpi = (short int)(descr->data.float_value * 100);
...
void op_proff() { printer_off(); }

/* ======================================================================
   op_pron()  -  PRINTER ON                                               */

void op_pron() { printer_on(); }
...
Private void phantom_error() { k_error(sysmsg(1711)); }
...
bool tio_display_string(char *q, short int bytes, bool flush,
                        bool transparent_newline) {
  short int n;
  char *p;
...
void tio_write(char *s) { tio_display_string(s, strlen(s), TRUE, FALSE); }

...
class HeaderFlags(object):
    ...
    def __str__(self):
        return ", ".join(
            [self._flags[1][i] for i, k in zip(
                range(16), self._flags[0]) if (self.flags & k == k)])
...
class CodeObject(object):
    ...
    def make_noxref(self):
        ...
        if sym_tab_off:
            obj_size = sym_tab_off
        line_tab_off = _header.line_table_offset
        if line_tab_off:
            obj_size = line_tab_off
        new_obj.change_header(
    ...
    def is_recursive(self):
        r = self._header.flags.recursive
        return os.path.basename(self.path)[0] == "_" and r
